...
#adam    # if (x$int.only) {
    ...
    if (!is.null(order)) {
        ...
        col <- col[sort.vec] #adam: reorder col
    }
    k <- length(yi)
    if (missing(rows)) {
        rows <- k:1
    ...
    if (addfit && !x$int.only) {
        for (i in seq_len(k)) {
            ...
            polygon(x = c(max(pred.ci.lb[i], alim[1]), pred[i],
                ...
                rows[i] - (height/100) * cex * efac[3]), col = col[i], #Adam: col now col [i]
                border = border, ...)
        }
    }
    if (addfit && x$int.only) {
        ...
        if (x$method != "FE" && addcred) {
            segments(max(beta.cr.lb, alim[1]), -1, min(beta.cr.ub,
                alim[2]), -1, lty = lty[2], col = col[i], ...) #col[2] now col[i]
        ...
        polygon(x = c(beta.ci.lb, beta, beta.ci.ub, beta), y = c(-1,
            -1 + (height/100) * cex * efac[3], -1, -1 - (height/100) *
                cex * efac[3]), col = col[i], border = border,  #col[1] now col[i]
            ...)
        if (missing(mlab))
    ...
    for (i in seq_len(k)) {
        ...
        if (ci.lb[i] >= alim[2]) {
            polygon(x = c(alim[2], alim[2] - (1.4/100) * cex *
                ...
                rows[i] + (height/150) * cex * efac[2], rows[i] -
                  (height/150) * cex * efac[2], rows[i]), col = col[i], #adam changed "black" to col[i]
                ...)
            next
        }
        if (ci.ub[i] <= alim[1]) {
        ...
        segments(max(ci.lb[i], alim[1]), rows[i], min(ci.ub[i], #adam: Adds CI line segment, we want this
            alim[2]), rows[i], lty = lty[1], col=col[i],...)
#adam: this stuff is for the line segments at the end of the CIs        # if (ci.lb[i] >= alim[1]) {
